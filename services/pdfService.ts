...
export const generateCMRPDF = (data: InvoiceData, products: Product[]) => {
    ...
    const splitAddress = doc.splitTextToSize(data.clientAddress.toUpperCase(), 80);
    doc.text(splitAddress, 20, 50);

    doc.setFontSize(12);
    doc.setFont('times', 'bold');
    doc.text(data.transport.toUpperCase(), 120, 45);
    if (data.transportAddress) {
        const splitTransAddress = doc.splitTextToSize(data.transportAddress.toUpperCase(), 80);
        doc.text(splitTransAddress, 120, 50);
    }
    ...
    doc.text(data.clientAddress, 20, 75);
...
export const generateNoteNavirePDF = (data: InvoiceData, products: Product[]) => {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(RED[0], RED[1], RED[2]);
    doc.text((data.cargador || data.transport).toUpperCase(), (20 + splitX) / 2, currentY + 36, { align: 'center' });
...
export const generateTransportInvoicePDF = (data: InvoiceData) => {
    ...
    autoTable(doc, {
        ...
        body: [
            [
                { 
                    content: `\nFRAIS DE TRANSPORT : ${trajet.toUpperCase()}\n\n\n\nC/R : ${data.trailer.toUpperCase()}`,
