...
const App: React.FC = () => {
    ...
    const [isTransportValidated, setIsTransportValidated] = useState(false);
    const [invoice, setInvoice] = useState<InvoiceData>({
        ...
        transportAddress: 'PORT DE PECHE TANGER', // Default address for default transport
        trailer: '',
        exchangeRate: 10.47,
        incoterm: 'FOB',
        items: [],
        ...
        transportAmount: 0,
        cargador: TRANSPORTS[0], // Default to transport
        trajet: ''
    });

    // Safety wrapper for local storage to prevent white page on corrupted data
    const safeParseStorage = (key: string, fallback: any) => {
        try {
    ...
    const [products, setProducts] = useState<Product[]>(() => safeParseStorage('dam_peche_products', DEFAULT_PRODUCTS));
    const [clients, setClients] = useState<Client[]>(() => safeParseStorage('dam_peche_clients', DEFAULT_CLIENTS));
    const [transports, setTransports] = useState<{name: string, address: string}[]>(() => {
        const saved = localStorage.getItem('dam_peche_transports_v2');
        if (saved) return JSON.parse(saved);
        return TRANSPORTS.map(t => ({ name: t, address: t === 'DAMJI TRANS SARL' ? 'PORT DE PECHE TANGER' : '' }));
    });
    ...
    useEffect(() => {
        localStorage.setItem('dam_peche_transports_v2', JSON.stringify(transports));
    }, [transports]);
    ...
    const handleTransportChange = (name: string) => {
        const t = transports.find(trans => trans.name === name);
        setInvoice(prev => ({
            ...prev,
            transport: name,
            transportAddress: t?.address || '',
            cargador: name // Sync cargador with transport by default
        }));
    };
    ...
    const addCustomTransport = () => {
        ...
        const t = { name: newTransport.name.toUpperCase(), address: newTransport.address };
        setTransports(prev => [...prev, t]);
        setInvoice(prev => ({ ...prev, transport: t.name, transportAddress: t.address, cargador: t.name }));
            ...
            <main className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-8 space-y-8">
                {activeTab === 'FACTURE' ? (
                        ...
                        <section className="no-print flex justify-center">
                            <div className="bg-white p-8 rounded-2xl shadow-md border-t-8 border-indigo-600 w-full max-w-5xl space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:col-span-3 lg:col-span-2">
                                        <div className="space-y-1">
                                            <div className="flex justify-between items-center">
                                                <label className="text-xs font-bold text-slate-500 uppercase tracking-tighter">Transport</label>
                                                <button onClick={() => setModalMode('transport')} className="text-[10px] text-indigo-600 font-black hover:underline">NOUVEAU</button>
                                            </div>
                                            <select value={invoice.transport} onChange={e => handleTransportChange(e.target.value)} className="w-full px-4 py-3 border-2 border-slate-100 rounded-xl focus:border-indigo-500 outline-none font-black text-indigo-900">
                                                {transports.map(t => <option key={t.name} value={t.name}>{t.name}</option>)}
                                            </select>
                                        </div>
                                        <div className="space-y-1">
                                            ...
                                            <input 
                                                ...
                                                className="w-full px-8 py-3 border-2 border-slate-200 rounded-xl focus:border-indigo-500 outline-none font-black text-xl text-indigo-900 placeholder:font-normal placeholder:text-slate-300 shadow-sm transition-all focus:ring-4 focus:ring-indigo-50" 
                                            />
                                        </div>
                                    </div>
                                    <div className="space-y-1 md:col-span-3">
                                        <label className="text-xs font-bold text-slate-500 uppercase tracking-tighter">Adresse Transporteur (pour CMR)</label>
                                        <input type="text" value={invoice.transportAddress} onChange={e => setInvoice(prev => ({...prev, transportAddress: e.target.value}))} className="w-full px-4 py-2 border-2 border-slate-100 rounded-xl focus:border-indigo-500 outline-none text-sm font-medium" placeholder="Laissez vide si pas d'adresse" />
                                    </div>
                                </div>
                            </div>
                        </section>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="no-print bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
                            <div className="flex items-center gap-2 text-indigo-800 font-black uppercase tracking-widest text-sm border-b pb-3">
                                <FileSignature className="w-5 h-5" />
                                Informations Complémentaires (Note de Navire)
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slate-500 uppercase tracking-tighter">Cargador o Agente de Aduanas (Chargeur ou transitaire)</label>
                                <input 
                                    type="text" 
                                    value={invoice.cargador} 
                                    onChange={e => setInvoice(prev => ({...prev, cargador: e.target.value}))} 
                                    ...
                                    <div className="grid grid-cols-12 mt-8 border-2 border-slate-900 min-h-[140px]">
                                        <div className="col-span-7 flex flex-col divide-y-2 divide-slate-900 border-r-2 border-slate-900">
                                            ...
                                            <div className="p-4 flex flex-col flex-1 justify-center text-center relative">
                                                ...
                                                <p className="text-xl font-black text-slate-900">{invoice.cargador}</p>
                                            </div>
                                        </div>
                                ...
                                <div className="max-w-4xl mx-auto bg-white p-20 rounded shadow-2xl min-h-[1100px] relative font-serif text-slate-900 border border-slate-200">
                                    ...
                                    <div className="grid grid-cols-2 mb-8 pt-4"> 
                                        <div className="space-y-0.5">
                                            ...
                                            <p className="text-sm font-bold uppercase whitespace-pre-line">{invoice.clientAddress}</p>
                                        </div>
                                        <div className="text-right space-y-0.5">
                                            <p className="font-bold text-lg uppercase">{invoice.transport}</p>
                                            <p className="text-sm uppercase whitespace-pre-line">{invoice.transportAddress}</p>
                                            <br/>
                                    ...
                                    <div className="mb-16 space-y-1">
                                        <p className="text-sm uppercase">{invoice.clientAddress}</p>
                                        <br/>
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-1">
                                        ...
                                        <input 
                                            ...
                                            onChange={e => {
                                                setInvoice(prev => ({...prev, transportInvoiceNumber: e.target.value}));
                                                setIsTransportValidated(false);
                                            }} 
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input 
                                                ...
                                                onBlur={e => {
                                                    setInvoice(prev => ({...prev, transportAmount: normalizeNum(e.target.value)}));
                                                    setIsTransportValidated(false);
                                                }} 
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <input 
                                            type="text" 
                                            placeholder="Ex: TANGER - VALENCIA" 
                                            value={invoice.trajet} 
                                            onChange={e => {
                                                setInvoice(prev => ({...prev, trajet: e.target.value}));
                                                setIsTransportValidated(false);
                                            }} 
                                            className="w-full px-4 py-3 border-2 border-slate-100 rounded-xl focus:border-indigo-500 outline-none font-bold" 
                                        />
                                    ...
                                    <button 
                                        onClick={() => {
                                            if (!invoice.trajet) {
                                                setInvoice(prev => ({...prev, trajet: `TANGER - ${destinationCity.toUpperCase()}`}));
                                            }
                                            setIsTransportValidated(true);
                                        }}
                            ...
                            <div className="bg-indigo-900 text-white p-8 rounded-3xl shadow-xl space-y-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="pb-4 border-b border-indigo-800">
                                        ...
                                        <p className="font-bold text-lg">{invoice.trajet || `TANGER - ${destinationCity.toUpperCase()}`}</p>
                                    </div>
                                    <div>
                            ...
                            <div className="max-w-4xl mx-auto bg-white p-12 rounded shadow-2xl min-h-[1100px] relative font-serif text-slate-900">
                                ...
                                <div className="w-full border-2 border-slate-900 mb-6">
                                    <div className="flex border-b-2 border-slate-900 font-bold bg-slate-50/20">
                                        <div className="w-[70%] py-1.5 text-center border-r-2 border-slate-900">DESIGNATION</div>
                                        <div className="w-[30%] py-1.5 text-center">MONTANT EUR</div>
                                    </div>
                                    <div className="flex min-h-[200px]">
                                        <div className="w-[70%] p-4 border-r-2 border-slate-900 relative">
                                            <p className="font-bold mb-8">FRAIS DE TRANSPORT : <span className="uppercase text-lg ml-2">{isTransportValidated ? invoice.trajet : (invoice.trajet || `TANGER - ${destinationCity}`)}</span></p>
                                            <p className="font-bold absolute bottom-8 left-4">C/R : {invoice.trailer.toUpperCase() || '18905 B 40 / 6962 08'}</p>
                                        </div>
                                    ...
                                    <div className="flex border-t-2 border-slate-900 font-bold">
                                        <div className="w-[70%] py-1 text-center border-r-2 border-slate-900">TOTAL</div>
                                        <div className="w-[30%] py-1 text-center">{formatEuro(invoice.transportAmount || 0)}</div>
                                    </div>
                                </div>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border animate-in zoom-in-95">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            {modalMode === 'transport' && (
                                <>
                                    <input type="text" value={newTransport.name} onChange={e => setNewTransport(prev => ({...prev, name: e.target.value}))} className="w-full px-4 py-2 border-2 border-slate-100 rounded-xl outline-none focus:border-indigo-500 font-bold" placeholder="Nom du Transporteur" />
                                    <input type="text" value={newTransport.address} onChange={e => setNewTransport(prev => ({...prev, address: e.target.value}))} className="w-full px-4 py-2 border-2 border-slate-100 rounded-xl outline-none focus:border-indigo-500" placeholder="Adresse (Optionnel)" />
                                </>
                            )}
                        </div>
